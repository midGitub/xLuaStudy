...
public class UnityWebRequestManager : SingletonBehaviour<UnityWebRequestManager>
{
    ...
    */
    ...
    /// <param name="actionResult">请求发起后处理回调结果的委托,处理请求对象</param>
    /// <returns></returns>
    public void DownloadBuffer(string url, Action<UnityWebRequest> actionResult)
    {
        StartCoroutine(_DownloadBuffer(url, actionResult));
    }
    ...
    /// <param name="actionResult">请求发起后处理回调结果的委托,处理请求对象</param>
    /// <returns></returns>
    IEnumerator _DownloadBuffer(string url, Action<UnityWebRequest> actionResult)
    {
        var uwr = new UnityWebRequest(url, UnityWebRequest.kHttpVerbGET);
